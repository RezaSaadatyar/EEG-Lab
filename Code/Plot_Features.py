...
def plot_features(data, data_features, Fs=None, channels=None, first_point=0, last_point=100, num_chan=None, type_feature=None):
    ...
    if Fs is not None and np.array(Fs) > 0:
        time = (np.linspace(start=first_point/Fs,
                stop=last_point/Fs, num=len(data))).flatten()
        axs[0].plot(time, data + 80*np.arange(data.shape[1], 0, -1))
        axs[0].set_xlabel('Time (sec)', fontsize=10)

        axs[1].plot(time, data_features + 80 *
                    np.arange(data_features.shape[1], 0, -1))
        axs[1].set_xlabel('Time (sec)', fontsize=10)
    else:
        axs[0].plot(data + 80*np.arange(data.shape[1], 0, -1))
        axs[0].set_xlabel('sample', fontsize=10)

        axs[1].plot(data_features + 80 *
                    np.arange(data_features.shape[1], 0, -1))
        axs[1].set_xlabel('sample', fontsize=10)

    axs[0].set_ylim([np.min(80*np.arange(data.shape[1], 0, -1))+np.min(data.iloc[:, data.shape[1]-1]),
                    np.max(80*np.arange(data.shape[1], 0, -1))+np.max(data.iloc[:, 0])])
    axs[1].set_ylim([np.min(80*np.arange(data_features.shape[1], 0, -1))+np.min(data_features.iloc[:, data_features.shape[1]-1]),
                    np.max(80*np.arange(data_features.shape[1], 0, -1))+np.max(data_features.iloc[:, 0])])
